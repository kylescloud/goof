/**
 * @file PancakeswapV3Adapter.ts
 * @description Adapter for PancakeSwap V3 on Base. Uses PancakeSwap's proprietary SmartRouter
 *              and Quoter V2. Handles PancakeSwap's fee tier differences (100, 500, 2500, 10000 bps).
 *              Builds V3-compatible calldata using PancakeSwap's ABI.
 */
...
export class PancakeswapV3Adapter extends BaseDexAdapter {
  readonly dexId = DexId.PANCAKESWAP_V3;
  ...
  constructor(provider: ethers.Provider, factoryAddress: string, routerAddress: string, quoterAddress: string) {
    super(provider, factoryAddress, routerAddress, 'PancakeswapV3Adapter');
    this.quoterAddress = quoterAddress;
  }

  async getAmountOut(params: SwapParams): Promise<bigint> {
    return this.retryCall(async () => {
      const quoter = this.getContract(this.quoterAddress, PANCAKE_QUOTER_ABI);
  ...
  async buildSwapCalldata(params: SwapParams, recipient: string, deadline: number): Promise<SwapCalldata> {
    const router = this.getContract(this.routerAddress, PANCAKE_ROUTER_ABI);
    const fee = params.fee ?? 2500;
    const minOut = params.minAmountOut ?? 0n;

    // PancakeSwap V3 SmartRouter uses a slightly different struct (no deadline in struct, uses multicall)
  ...
  /**
   * Gets the best fee tier for a given pair by trying all PancakeSwap fee tiers.
   */
  async getBestFeeTier(tokenIn: string, tokenOut: string, amountIn: bigint): Promise<{ fee: number; amountOut: bigint }> {
