 ...
 * @description Full adapter for Uniswap V2 (Base). Implements getAmountOut using on-chain
 *              getReserves combined with V2Math. Implements buildSwapCalldata for
 *              swapExactTokensForTokens. Implements getPoolReserves for the discovery engine.
 */
...
export class UniswapV2Adapter extends BaseDexAdapter {
  readonly dexId = DexId.UNISWAP_V2;
  ...
  constructor(provider: ethers.Provider, factoryAddress: string, routerAddress: string) {
    super(provider, factoryAddress, routerAddress, 'UniswapV2Adapter');
  }
