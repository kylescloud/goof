/**
 * @file UniswapV3Adapter.ts
 * @description Full adapter for Uniswap V3 (Base). Implements getAmountOut by calling the
 *              V3 Quoter's quoteExactInputSingle(). Implements buildSwapCalldata for
 *              exactInputSingle and exactInput. Reads slot0() and liquidity() for pool state.
 */
...
export class UniswapV3Adapter extends BaseDexAdapter {
  readonly dexId = DexId.UNISWAP_V3;
  ...
  constructor(provider: ethers.Provider, factoryAddress: string, routerAddress: string, quoterAddress: string) {
    super(provider, factoryAddress, routerAddress, 'UniswapV3Adapter');
  ...
  async getAmountOut(params: SwapParams): Promise<bigint> {
    return this.retryCall(async () => {
      ...
      try {
        ...
        // Fallback: use on-chain pool state for approximation
  ...
  private async _approximateFromPoolState(params: SwapParams): Promise<bigint> {
    try {
      ...
      const [slot0Data, liq, token0] = await Promise.all([
        pool.slot0(),
        pool.liquidity(),
        pool.token0(),
      ]);
