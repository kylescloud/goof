...
export class TwoHopCrossDex extends BaseStrategy {
  ...
  async findOpportunities(): Promise<ArbitragePath[]> {
    ...
    for (const [, poolGroup] of pairPools) {
      ...
      for (let i = 0; i < poolGroup.length; i++) {
        for (let j = 0; j < poolGroup.length; j++) {
          ...
          if (sellOutput <= totalRepayment) continue;

          const profit = await this.estimateNetProfitUsd(
            flashAsset, flashAmount, sellOutput, decimalsB,
            [this._poolToEdge(buyPool, tokenB, tokenA), this._poolToEdge(sellPool, tokenA, tokenB)]
          );
    ...
    this.logger.info('Two-hop cross-DEX scan complete', { opportunities: opportunities.length });
